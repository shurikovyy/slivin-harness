# Архитектура Slivin Harness 0.8.0a37 — Phase 7

## Назначение

`0.8.0a37` требует typed impact closure в `planner.v6` до `READY`, сохраняет
...
`harness-build-identity.v1`: package version `0.8.0a37`, exact Git HEAD и tracked
...
`0.8.0a37` не утверждает универсальный OS-enforced sandbox для любого Controller subprocess. Owner-configured external wrappers обязаны сами иметь scoped credential/environment boundary.
...
а session cwd — уникальный `.harness_tmp/<role>/session-*`. Named permission profile
даёт filesystem root read и только этому session root write; network выключен,
approvalPolicy=never. Команды явно используют project workdir без дополнительного
write grant. TEMP/TMP/TMPDIR и cache environment задаются per-thread; общий App Server
environment не перенастраивается между ролями. Turns наследуют проверенный thread
context без cwd/permission overrides: в 0.153.4 повторный выбор profile на turn/start
перезагружает base config без thread-local profile и отклоняется.
...
Executable boundary identity не доверяет runtime `function.__module__`.
`entrypoint_identity.py` canonicalize resolved existing `.py` source внутри Harness
root в dotted module и добавляет normalized lexical qualname; static AST inventory
использует тот же resolver. Поэтому import, `python file.py` и runpy-style
`__main__` имеют одну identity, а outside-root/non-module sources fail closed.
Исторический global alias `__main__ -> task_runner` отсутствует. B21 state machine
...
`CodexAppServer` передаёт оба значения explicit CLI `-c`, поэтому ambient user
config не может переопределить профиль. Mode/model/effort/Codex version,
requested/executed cases и fail-fast policy входят в qualification и per-case evidence.
...
boot failure останавливает release до `native_roles`.
...
threads, сохраняя rollout и освобождая session cwd. Scoped sessions materialized:
0.153.4 не поддерживает archive для ephemeral thread. Intake/Implementer сохраняют
прежний ephemeral setting. Повторный turn на retired thread
запрещён. Затем очищаются только проверенные role roots;
Windows extended-length paths поддерживают длинные Jest cache names, cleanup errors
не скрываются. Corrective Planner и Evaluator A/B сохраняют свой root.
Порядок blind persistence/disclosure и candidate/Git/runtime guards не меняется.

Предварительный `execution_policies.json` описывает role allocation boundary,
не grant на весь parent scratch. Конкретный единственный writable root выбирается
только per-thread context.

`harness_execution_context` в thread metadata отделяет requested policy, reported
effective projection и `filesystem_probe_result=NOT_RUN_BY_THREAD_START`.
Несовместимый runtime/ответ даёт typed `role_execution_policy_failure.json`, без
небезопасного fallback. Native enforcement проверяется отдельно
[воспроизводимым smoke](WINDOWS_SETUP.md#native-scoped-scratch-acceptance); D-018
в [журнале решений](DECISIONS.md) фиксирует ограничения и отвергнутые механизмы.
