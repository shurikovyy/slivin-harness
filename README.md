# Slivin Harness 0.8.0a39 — Phase 7
...
**Начните с [Autonomous Engineering Contract](docs/AUTONOMOUS_ENGINEERING_CONTRACT.md)** —
нормативной обязанности агента самостоятельно исследовать root cause и полный
...
`0.8.0a39` quality-core требует typed Planner Impact Closure до `READY` в `planner.v6`
...
captured Codex transport failures, captured native role-command drift и четыре
captured real-model liveness failures, включая pruning-only recovery отсутствующего
repository path в Evaluator Phase A, до
...
самого Planner не разрешают исключение. Сохраняются strict-валидация agent output schemas до
`turn/start`, capability-aware planning и Stage 0 static toolchain gate:
...
diagnostic. `harness_build_identity.json` связывает run с `0.8.0a39`, exact Git
...
Каждый trusted check и Harness-owned self-verify также получает одноразовый
...
```text
DOCS_SYNC_PASS harness=0.8.0a39 ...
...
`0.8.0a39` завершает согласованный Step 0–7 quality-core, но намеренно **не заявляет готовыми**:
